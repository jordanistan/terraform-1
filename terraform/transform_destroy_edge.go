	...
	for {
		...
		for i := 0; i < len(nodes); i++ {
			...
			func() {
				...
				// If we removed the expander node, we won't be able to lookup
				// the instances in the instance expander. We still need the
				// closer to clean up state and act as the root node for the
				// module, but we need to disable it from looking up module
				// instances.
				if _, ok := n.(*nodeExpandModule); ok {
					if m.closer != nil {
						m.closer.destroy = true
					}
				}
